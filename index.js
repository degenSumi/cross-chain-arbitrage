...
const connection = new Connection(solrpc, "finalized");
...
async function runbot() {
  ...
  listener.on("solpool", async (poolinfo) => {
    ...
    // return;
    const pricediff =
      poolinfo.currentPriceOnSol - poolInfoSui.currentPriceOnSui;
    switch (true) {
      case Math.abs(pricediff) >= config.threshold && pricediff < 0:
        ...
        try {
          ...
          const outAmountInUsdc =
            destinationSwap.swapOutAmount / 10 ** sui_pool.token_1_decimals; // Sus slippage
          let networkCost = Number(2 * 5000); // 2 signatures for bridge and swap 5000 lamports*2
          let networkCostInUsdc =
            (networkCost / 10 ** 9) * Number(poolinfo.currentPriceOnSol);
          // add the swap gas on destination chain
          networkCostInUsdc +=
            (destinationSwap.gasUsed / 10 ** 9) * poolInfoSui.suiNativePrice;
          ...
          if (arbValueUSDC <= 0) {
            console.log(
              chalk.red(`🚨 Arbitrage Opportunity Detected! 
                        - Estimated Loss: **${arbValueUSDC.toFixed(3)} USDC**
                        - Trade Amount: **${config.swapamount.toFixed(2)} ${
                config.token
              }**`)
            );
          } else {
            ...
            const tx = await web3.sendAndConfirmTransaction(
              ...
              [
                Keypair.fromSecretKey(
                  new Uint8Array(bs58.decode(process.env.solanaprivatekey))
                ),
              ]
            );
      ...
      case Math.abs(pricediff) >= config.threshold && pricediff > 0:
        try {
          ...
          const outAmount = Number(bridgequote.destinationToken.amount);
          const outAmountInUsdc =
            destinationSwap.swapOutAmount / 10 ** sol_pool.token_1_decimals;
          ...
          if (arbValueUSDC > 0.1) {
            ...
            const bridgeTx = await startbridge({
              ...
              rcvChain: "Solana",
              amount:
                sourceSwap.swapOutAmount / 10 ** sui_pool.token_0_decimals,
              solprivatekey,
              suiprivatekey,
