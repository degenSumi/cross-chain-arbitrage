...
async function startbridge(options) {
  ...
  let source =
    options.sendChain === "Solana"
      ? await solanasign.getSigner(
          await sendChain.getRpc(),
          options.solprivatekey,
          {
            debug: true,
            priorityFee: {
      ...
      : await suisign.getSigner(
          await sendChain.getRpc(),
          options.suiprivatekey
        );
  ...
  let destination =
    options.rcvChain === "Solana"
      ? await solanasign.getSigner(
          await rcvChain.getRpc(),
          options.solprivatekey,
          {
            debug: true,
            priorityFee: {
...
async function tokenTransfer(wh, route, options) {
  ...
  if (!options.execute) return quote;
